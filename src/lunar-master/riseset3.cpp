                  ...
                  const char *vsop_data, int *is_setting)
{
   ...
   if( *is_setting != -1)
      ...
      while( fabs( delta) > .0001 && iterations--)
         ...
         double ra = (pdata.ecliptic_lon * 180.);
         double dec = (pdata.ecliptic_lat * 180.) / 24.;
//         fprintf(stderr, "RA and dec for %d is %f and %f\n", planet_no, ra, dec);
 //        fprintf(stderr, "RA and dec for %lf is %lf and %lf\n", planet_no, pdata.ecliptic_loc[0], pdata.ecliptic_loc[1], pdata.ecliptic_loc[2]);
